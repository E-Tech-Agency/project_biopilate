...
def generateOtp():
    otp = ''.join(str(random.randint(0, 9))
                  for _ in range(6))  # Generate a 6-digit OTP
    return otp
...
def send_code_to_user(email):
    ...
    OneTimePassword.objects.create(
        user=user, code=otp_code)  # Store OTP in the database
    ...
    try:
        # Set to False to raise exceptions if email fails to send
...
def send_normal_email(data):
    ...
    try:
        # Raise exceptions if email fails to send
...
def send_contact_form_email(name, email, phone, subject, message):
    ...
    try:
        send_mail(
            email_subject,
            email_message,
            'carolinebergerdefemynie@gmail.com',  # From email (OVH email)
            # To email (where you receive contact form submissions)
            ['carolinebergerdefemynie@gmail.com'],
            fail_silently=False,
        )
    except Exception as e:
